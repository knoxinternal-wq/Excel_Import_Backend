...
function canonicalizeBrand(brandRaw) {
  ...
  return b; // unsupported brand (caller validates against legacy table map)
}
...
async function ensureLegacySoMasterTables(client) {
  const tableSpecs = [
    { name: 'dnj_so_master', company: 'DNJ' },
    { name: 'ic_so_master', company: 'IC' },
    { name: 'rf_so_master', company: 'RF' },
    { name: 'vercelli_so_master', company: 'VERCELLI' },
  ];
  for (const t of tableSpecs) {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS ${t.name} (
        ...
        so_agent_name TEXT,
        branch TEXT,
        company_name TEXT DEFAULT '${t.company}',
        so_order_no TEXT,
        so_order_date DATE,
        type_of_order TEXT,
        brand TEXT,
    ...
    // eslint-disable-next-line no-await-in-loop
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_${t.name}_lookup ON ${t.name} (party_name, brand, fy);
      CREATE INDEX IF NOT EXISTS idx_${t.name}_order_date ON ${t.name} (so_order_date);
      CREATE INDEX IF NOT EXISTS idx_${t.name}_agent ON ${t.name} (so_agent_name);
    `);
  }
}
...
async function ensureUploadHistoryTable(client) {
  ...
  `);
  await client.query(`
    ALTER TABLE ${HISTORY_TABLE} ADD COLUMN IF NOT EXISTS filename TEXT;
    ALTER TABLE ${HISTORY_TABLE} ADD COLUMN IF NOT EXISTS fy TEXT;
    ALTER TABLE ${HISTORY_TABLE} ADD COLUMN IF NOT EXISTS status TEXT;
    ALTER TABLE ${HISTORY_TABLE} ADD COLUMN IF NOT EXISTS inserted_rows INTEGER DEFAULT 0;
    ALTER TABLE ${HISTORY_TABLE} ADD COLUMN IF NOT EXISTS error_message TEXT;
  `);
  await client.query(`
...
export async function importSoMaster(req, res) {
  ...
  try {
    ...
    try {
      await ensureLegacySoMasterTables(client);
      await ensureUploadHistoryTable(client);
      await client.query('BEGIN');
      if (!legacyTable) {
        throw new Error(`Unsupported brand: ${brandRaw}`);
      }

      for (let i = 0; i < dedupedValid.length; i += bulkChunkSize) {
        const chunk = dedupedValid.slice(i, i + bulkChunkSize);
        // eslint-disable-next-line no-await-in-loop
        await syncRowsToLegacyTable({ rows: chunk, tableName: legacyTable, legacyBrand: brand });
